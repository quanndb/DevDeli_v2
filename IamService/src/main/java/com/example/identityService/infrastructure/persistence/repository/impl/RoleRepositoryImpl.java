...
import com.example.identityService.infrastructure.persistence.repository.CustomRoleRepository;
...
public class RoleRepositoryImpl implements CustomRoleRepository {
    ...
    public List<RoleResponse> search(RolePageRequest request) {
        ...
        String sql = "select a from RoleEntity a " + createWhereQuery(request, values) + createOrderQuery(request.getSortedBy(), request.getSortDirection());
    ...
    public Long count(RolePageRequest request) {
        Map<String, Object> values = new HashMap<>();
        String sql = "select count(a) from RoleEntity a " + createWhereQuery(request, values);
