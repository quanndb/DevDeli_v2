package com.example.identityService.infrastructure.persistence.repository.impl;
...
import com.example.identityService.infrastructure.persistence.repository.CustomPermissionRepository;
...
public class PermissionRepositoryImpl implements CustomPermissionRepository {
    ...
    public List<PermissionResponse> search(PermissionPageRequest request) {
        ...
        String sql = "select a from PermissionEntity a " + createWhereQuery(request, values) + createOrderQuery(request.getSortedBy(), request.getSortDirection());
        Query query = entityManager.createQuery(sql, PermissionEntity.class);
    ...
    public Long count(PermissionPageRequest request) {
        Map<String, Object> values = new HashMap<>();
        String sql = "select count(a) from PermissionEntity a " + createWhereQuery(request, values);
