...
import com.example.identityService.infrastructure.persistence.repository.CustomAccountRepository;
...
public class AccountRepositoryImpl implements CustomAccountRepository {
    ...
    public List<UserResponse> search(UserPageRequest request) {
        ...
        String sql = "select a from AccountEntity a " + createWhereQuery(request, values) + createOrderQuery(request.getSortedBy(), request.getSortDirection());
        Query query = entityManager.createQuery(sql, AccountEntity.class);
    ...
    public Long count(UserPageRequest request) {
        Map<String, Object> values = new HashMap<>();
        String sql = "select count(a) from AccountEntity a " + createWhereQuery(request, values);
